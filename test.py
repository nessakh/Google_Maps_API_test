...
def Main():
	...
	parser.add_argument("starter", help="the starter is $")
	parser.add_argument("name", help="vanessa's foursquare")
	...
	if not args.name == './vanessa_foursquare' or not args.starter == '$':
		print('Syntax Error : expression should start with $ ./vanessa_foursquare ')
	else:
...
def location_search(radius, categorie):
	...
	final_url = url + "&radius=" + radius + '&type=' + categorie + '&key=' + key
	print final_url
...
def get_name(placeid):
	...
	json_obj = urllib2.urlopen(final_url)
	data = json.load(json_obj)
	print data["result"]["address_components"]["long_name"]  
Main()
